...
table_name_dict = {layer: f"source_a.{test_identifier}_{layer}_table" 
                   for layer in layers}
...
class MyTestFixture(NutterFixture):
   ...
   def run_source_a_end_to_end(self):
      (dbutils.notebook
       .run('../src/source_A_1_raw_to_bronze_notebook', 600, bronze_args))
      (dbutils.notebook
       .run('../src/source_A_2_bronze_to_silver_notebook', 600, silver_args))
      (dbutils.notebook
       .run('../src/source_A_3_silver_to_gold_notebook', 600, gold_args))
